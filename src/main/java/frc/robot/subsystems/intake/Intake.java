...
public class Intake extends SubsystemBase {
  ...
  private final TrapezoidProfile profile =
      new TrapezoidProfile(
          new TrapezoidProfile.Constraints(
              armMaxVelocityRadPerSec, armMaxAccelerationRadPerSecSquared));
  ...
  public Intake(IntakeIO io) {
    this.io = io;
    timer.start();
    setpoint = new TrapezoidProfile.State(armRetractedPosition.getRadians(), 0.0);
    goal = new TrapezoidProfile.State(armRetractedPosition.getRadians(), 0.0);
  }
  ...
  public void periodic() {
    ...
    io.setArmPosition(new Rotation2d(setpoint.position), feedforwardOutput);
  ...
  public void deploy() {
    goal = new TrapezoidProfile.State(armDeployedPosition.getRadians(), 0.0);
